...
function OwnerContactTab() {
  ...
  const handleChange = (key: keyof OwnerContactState, value: string) => {
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      // Stelle sicher, dass alle Felder korrekt gesetzt werden, auch wenn sie leer sind
      ...
      if (updated && updated[0]) {
        console.log('Profil erfolgreich aktualisiert:', updated[0]);
        updateProfileState(updated[0]);
        // Stelle sicher, dass der lokale State auch aktualisiert wird
        setState({
          street: updated[0].street || '',
          plz: updated[0].plz || '',
          city: updated[0].city || '',
      ...
      console.error('Unerwarteter Fehler beim Speichern:', e);
